...
struct Ray
{
	...
	__device__  inline Ray(glm::vec3 org, glm::vec3 dir, const int index)
	{
		this->origin = org;
		this->direction = dir;
	...
	__device__ inline static Ray generate(const glm::vec3& origin, const glm::vec3& viewDir, const glm::vec3& horizontal, const glm::vec3& vertical, const float x, const float y, const float invw, const float invh, const int index)
	{
		...
		return Ray(origin, glm::normalize(p), index);
	}
};
